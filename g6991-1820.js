...
function add_img(id01,id02,ww,hh,pp,xx,yy,zz,fname,oth) {
 ...
 fname=bmpid[bmport]+'/'+fname;
 bmport=bmport+1;
 if (bmport >= bmpid.length ) { bmport=0; }
 var b2='<img src='+ fname +' style="width:100%; height:100%">';
...
function httpGet(fname) { 
 var getxt = new XMLHttpRequest(); 
 getxt.open( "GET", fname, false ); 
 getxt.send( null );
 return getxt.responseText;
}

function insertObjtag(id01,id02,fname) { 
 //insertnull(id02);
 fname=serid[sport]+'/'+fname;
 sport=sport+1;
...
function checkImgtag(idata) { 

 var res=idata.split('<img');
 
for (i=1; i<=(res.length-1); i++) {
 var txt='src='+bmpid[bmport]+'/';
 bmport=bmport+1;
 if (bmport >= bmpid.length ) { bmport=0; }
 res[i]='<img'+res[i];
 var fin=res[i].substring(res[i].indexOf("<img"),res[i].indexOf("</img>")+6 ); 
 var finn=fin.replace("src=",txt);
 var idata=idata.replace(fin,finn); 
} 
 return idata;
} 

function saveData(idid,fname) { 
  var idata;
  idata=document.getElementById(idid).value;
  localStorage.fname=idata;
} 

function loadData(idid,fname) { 
  var idata;
  idata=localStorage.fname;
 document.getElementById(idid).value=idata;
} 
